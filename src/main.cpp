...
const int loglevel = QMMLOG_DEBUG;
...
char* get_args() {
	...
	for (intptr_t argnum = 1; argnum < argc; argnum++) {
		QMM_ARGV(argnum, arg, sizeof(arg));
	...
	while (find) {
		if (*find == '%') {
			*find = '@';
			...
			f = (*find == '\n' ? 'n' : 'r');
			*find = '\0';
			strncpyz(buf, QMM_VARARGS("%s\\%c%s", buf, f, find + 1), sizeof(buf));
		}
		find = strpbrk(find, "\n\r");
	}
...
C_DLLEXPORT intptr_t QMM_vmMain(intptr_t cmd, intptr_t* args) {
	...
	const char* msgname = QMM_MODMSGNAME(cmd);
	...
	if (cmd == GAME_CONSOLE_COMMAND) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain(%s(%d)) :: args: \"%s\" :: current return value: %d :: highest result: %s\n", msgname, cmd, str_escape(get_args()), current_return_value, highest_result), loglevel);
	}
...
// these games return a bool rather than a string
			...
			QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain(%s(%d), %d) :: current return value: %d :: highest result: %s\n", msgname, cmd, clientNum, current_return_value, highest_result), loglevel);
#else
			QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain(%s(%d), %d) :: current return value: \"%s\" :: highest result: %s\n", msgname, cmd, clientNum, (char*)current_return_value, highest_result), loglevel);
#endif
		}
		else if (cmd == GAME_CLIENT_COMMAND) {
			QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain(%s(%d), %d) :: args: \"%s\" :: current return value: %d :: highest result: %s\n", msgname, cmd, clientNum, str_escape(get_args()), current_return_value, highest_result), loglevel);
		}
		else {
			QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain(%s(%d), %d) :: highest result: %s\n", msgname, cmd, clientNum, highest_result), loglevel);
		}
	}
...
// MOH?? have a SpawnEntities function but no mapname
		...
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain(%s(%d), \"%s\") :: highest result: %s\n", msgname, cmd, (char*)args[0], highest_result), loglevel);
	}
#endif
	else {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain(%s(%d)) :: current return value: %d :: highest result: %s\n", msgname, cmd, current_return_value, highest_result), loglevel);
	}
...
C_DLLEXPORT intptr_t QMM_vmMain_Post(intptr_t cmd, intptr_t* args) {
	...
	if (cmd == GAME_CONSOLE_COMMAND) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain_Post(%s(%d)) :: args: \"%s\" :: current return value: %d :: original return value: %d :: highest result: %s\n", msgname, cmd, get_args(), current_return_value, original_return_value, highest_result), loglevel);
	}
	else if (cmd == GAME_CLIENT_CONNECT || cmd == GAME_CLIENT_COMMAND || cmd == GAME_CLIENT_BEGIN ||
		cmd == GAME_CLIENT_USERINFO_CHANGED || cmd == GAME_CLIENT_DISCONNECT || cmd == GAME_CLIENT_THINK) {
		intptr_t clientNum = args[0];
...
#if defined(GAME_CLIENT_CONNECT_RETURNS_BOOL)
			QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain_Post(%s(%d), %d) :: current return value: %d :: original return value: %d :: highest result: %s\n", msgname, cmd, clientNum, current_return_value, original_return_value, highest_result), loglevel);
#else
			QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain_Post(%s(%d), %d) :: current return value: \"%s\" :: original return value: \"%s\" :: highest result: %s\n", msgname, cmd, clientNum, (const char*)current_return_value, (const char*)original_return_value, highest_result), loglevel);
#endif
		}
		else if (cmd == GAME_CLIENT_COMMAND) {
			QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain_Post(%s(%d), %d) :: args: \"%s\" :: current return value: %d :: original return value: %d :: highest result: %s\n", msgname, cmd, clientNum, get_args(), current_return_value, original_return_value, highest_result), loglevel);
		}
		else {
			QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain_Post(%s(%d), %d) :: highest result: %s\n", msgname, cmd, clientNum, highest_result), loglevel);
		}
	}
	// mapname is the first argument to SpawnEntities or Init in all games with this feature flag
	// MOH?? have a SpawnEntities function but no mapname
		...
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain_Post(%s(%d), \"%s\") :: highest result: %s\n", msgname, cmd, (char*)args[0], highest_result), loglevel);
	}
#endif
	else {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_vmMain_Post(%s(%d)) :: current return value: %d :: original return value: %d :: highest result: %s\n", msgname, cmd, current_return_value, original_return_value, highest_result), loglevel);
	}
...
C_DLLEXPORT intptr_t QMM_syscall(intptr_t cmd, intptr_t* args) {
	...
	const char* msgname = QMM_ENGMSGNAME(cmd);
	intptr_t current_return_value = QMM_VAR_RETURN(intptr_t);
	const char* highest_result = plugin_result_to_str(QMM_VAR_HIGH_RES());

	if (cmd == G_PRINT || cmd == G_ERROR) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall(%s(%d), \"%s\") :: highest result: %s\n", msgname, cmd, str_escape((char*)args[0]), highest_result), loglevel);
	}
	else if (cmd == G_CVAR_VARIABLE_STRING_BUFFER) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall(%s(%d), \"%s\") :: highest result: %s\n", msgname, cmd, (char*)args[0], highest_result), loglevel);
	}
	else if (cmd == G_CVAR_VARIABLE_INTEGER_VALUE || cmd == G_FS_FOPEN_FILE) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall(%s(%d), \"%s\") :: current return value: %d :: highest result: %s\n", msgname, cmd, (char*)args[0], current_return_value, highest_result), loglevel);
	}
	else if (cmd == G_CVAR_SET) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall(%s(%d), \"%s\", \"%s\") :: highest result: %s\n", msgname, cmd, (char*)args[0], (char*)args[1], highest_result), loglevel);
	}
	else if (cmd == G_CVAR_REGISTER) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall(%s(%d), \"%s\", \"%s\") :: highest result: %s\n", msgname, cmd, (char*)args[1], (char*)args[2], highest_result), loglevel);
	}
	else if (cmd == G_LOCATE_GAME_DATA) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall(%s(%d), %p, %d, %d, %p, %d) :: highest result: %s\n", msgname, cmd, (void*)args[0], args[1], args[2], (void*)args[3], args[4], highest_result), loglevel);
	}
	else {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall(%s(%d)) :: current return value: %d :: highest result: %s\n", msgname, cmd, current_return_value, highest_result), loglevel);
	}
...
C_DLLEXPORT intptr_t QMM_syscall_Post(intptr_t cmd, intptr_t* args) {
	...
	if (cmd == G_ARGV) {
		...
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall_Post(%s(%d), %d) :: current return value: \"%s\" :: original return value: \"%s\" :: highest result: %s\n", msgname, cmd, args[0], (char*)current_return_value, (char*)original_return_value, highest_result), loglevel);
#else
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall_Post(%s(%d), %d, \"%s\") :: highest result: %s\n", msgname, cmd, args[0], (char*)args[1], highest_result), loglevel);
#endif
	}
	else if (cmd == G_ARGS) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall_Post(%s(%d)) :: current return value: \"%s\" :: original return value: \"%s\" :: highest result: %s\n", msgname, cmd, (char*)current_return_value, (char*)original_return_value, highest_result), loglevel);
	}
	else if (cmd == G_GET_ENTITY_TOKEN || cmd == G_CVAR_VARIABLE_INTEGER_VALUE || cmd == G_FS_FOPEN_FILE) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall_Post(%s(%d), \"%s\") :: current return value: %d :: original return value: %d :: highest result: %s\n", msgname, cmd, (const char*)args[0], current_return_value, original_return_value, highest_result), loglevel);
	}
	else if (cmd == G_PRINT || cmd == G_ERROR) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall_Post(%s(%d), \"%s\") :: highest result: %s\n", msgname, cmd, str_escape((char*)args[0]), highest_result), loglevel);
	}
	else if (cmd == G_CVAR_VARIABLE_STRING_BUFFER || cmd == G_CVAR_SET) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall_Post(%s(%d), \"%s\", \"%s\") :: highest result: %s\n", msgname, cmd, (char*)args[0], (char*)args[1], highest_result), loglevel);
	}
	else if (cmd == G_CVAR_REGISTER) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall_Post(%s(%d), \"%s\", \"%s\") :: highest result: %s\n", msgname, cmd, (char*)args[1], (char*)args[2], highest_result), loglevel);
	}
	else if (cmd == G_LOCATE_GAME_DATA) {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall_Post(%s(%d), %p, %d, %d, %p, %d) :: highest result: %s\n", msgname, cmd, (void*)args[0], args[1], args[2], (void*)args[3], args[4], highest_result), loglevel);
	}
	else {
		QMM_WRITEQMMLOG(QMM_VARARGS("QMM_syscall_Post(%s(%d)) :: current return value: %d :: original return value: %d :: highest result: %s\n", msgname, cmd, current_return_value, original_return_value, highest_result), loglevel);
	}
